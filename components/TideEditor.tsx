...
export const TideEditor: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full gap-4 overflow-hidden">
      ...
      <div className="shrink-0 flex-[0_0_35%] min-h-[180px] bg-slate-800 rounded-lg border border-slate-700 p-1 relative flex flex-col shadow-inner overflow-hidden">
        <div className="absolute top-2 left-4 z-10 bg-slate-900/80 px-2 py-0.5 rounded border border-slate-700 backdrop-blur-sm">
        ...
        <div className="absolute top-2 right-4 z-10 flex gap-4 bg-slate-900/80 px-2 py-0.5 rounded border border-slate-700 backdrop-blur-sm">
      ...
      {/* Use flex-1 with min-h-0 to allow nested scrolling without pushing parent bounds */}
      <div className="flex-1 min-h-0 grid grid-cols-1 xl:grid-cols-3 gap-4 overflow-hidden">
          ...
          <div className="xl:col-span-2 bg-slate-800 rounded-lg border border-slate-700 p-4 flex flex-col overflow-hidden min-h-0">
              <h3 className="text-xs font-bold text-white mb-3 flex items-center gap-2 border-b border-slate-700 pb-2 shrink-0">
                   <Database size={14} className="text-purple-400"/> Fonte de Dados
              </h3>
              
              <div className="flex gap-2 mb-3 bg-slate-900/50 p-1 rounded-lg w-fit overflow-x-auto shrink-0 max-w-full">
                   {[
                       ...
                       { type: TideSourceType.MOCK, label: "Mockados", icon: <Activity size={12}/>, color: "text-green-400", bg: "bg-green-500/20", border: "border-green-500/30" },
                   ].map(opt => (
                       <button 
                          key={opt.type}
                          onClick={() => updateDataSourceConfig({ activeSource: opt.type })}
                          className={`px-3 py-1.5 rounded text-[10px] font-bold flex items-center gap-2 transition whitespace-nowrap ${dataSourceConfig.activeSource === opt.type ? `${opt.bg} ${opt.color} border ${opt.border}` : 'text-slate-400 hover:text-slate-200'}`}
                       >
                           {opt.icon} {opt.label}
                       </button>
                   ))}
              ...
              {/* CONFIG PANEL - DYNAMIC & SCROLLABLE */}
              <div className="flex-1 bg-slate-900 rounded-lg border border-slate-700 p-4 overflow-y-auto custom-scrollbar min-h-0">
                  
                  {dataSourceConfig.activeSource === TideSourceType.TABUA_MARE && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in">
                          <div className="space-y-3">
                              <div>
                                  <label className="text-[9px] text-slate-500 uppercase font-bold">ID do Porto (Recomendado)</label>
                                  <div className="flex gap-2 mt-1">
                                    <input 
                                        ...
                                        className="flex-1 bg-slate-800 border border-slate-600 rounded p-1.5 text-xs text-white focus:border-yellow-500 outline-none"
                                    />
                                     <button 
                              ...
                              <p className="text-[9px] text-slate-500 leading-tight">Se ID nulo, busca por coordenadas geográficas.</p>
                          </div>
                          <div className="space-y-3">
                              <div className="flex gap-2">
                                  <div className="flex-1">
                                      <label className="text-[9px] text-slate-500 uppercase font-bold">Latitude</label>
                                      <input type="number" value={dataSourceConfig.tabuaMare.lat} onChange={(e) => updateDataSourceConfig({ tabuaMare: {...dataSourceConfig.tabuaMare, lat: parseFloat(e.target.value)} })} className="w-full mt-1 bg-slate-800 border border-slate-600 rounded p-1.5 text-xs text-white"/>
                                  </div>
                                  <div className="flex-1">
                                      <label className="text-[9px] text-slate-500 uppercase font-bold">Longitude</label>
                                      <input type="number" value={dataSourceConfig.tabuaMare.lng} onChange={(e) => updateDataSourceConfig({ tabuaMare: {...dataSourceConfig.tabuaMare, lng: parseFloat(e.target.value)} })} className="w-full mt-1 bg-slate-800 border border-slate-600 rounded p-1.5 text-xs text-white"/>
                                  </div>
                              </div>
                              <div className="flex gap-2 pt-2">
                                <button 
                                    ...
                                    className="flex-1 bg-yellow-600 hover:bg-yellow-500 text-white font-bold py-2 rounded text-xs flex items-center justify-center gap-2 transition shadow-lg"
                                >
                                    {isGeneratingSource ? <RefreshCw className="animate-spin" size={12} /> : <Server size={12} />} Buscar Dados
                                </button>
                                <button 
                      ...
                      <div className="space-y-3 animate-in fade-in">
                           ...
                           <div className="flex gap-2 pt-2">
                                <button 
                                        onClick={handleSourceGenerate}
                                        disabled={isGeneratingSource}
                                        className="flex-1 bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-2 rounded text-xs flex items-center justify-center gap-2 transition shadow-lg"
                                    >
                                        {isGeneratingSource ? <RefreshCw className="animate-spin" size={12} /> : <Server size={12} />} Buscar Dados
                                </button>
                                <button 
                  ...
                  {dataSourceConfig.activeSource === TideSourceType.CALCULATED && (
                      <div className="animate-in fade-in flex flex-col gap-4">
                          {/* VISUAL FORMULA */}
                          <div className="bg-black/50 p-3 rounded-lg border border-slate-700 font-mono text-center flex flex-col items-center justify-center gap-1 shrink-0">
                              <div className="text-[9px] text-slate-500 uppercase tracking-widest">Matemática de Cálculo</div>
                              <div className="text-xs md:text-sm text-pink-400 break-words w-full px-2">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                              <div className="bg-slate-800/50 p-2 rounded">
                                  <label className="text-[9px] text-slate-500 uppercase font-bold block mb-1">Período (Horas)</label>
                              ...
                              <div className="bg-slate-800/50 p-2 rounded">
                                  <label className="text-[9px] text-slate-500 uppercase font-bold block mb-1">Amplitude (%)</label>
                                  <div className="flex items-center gap-2">
                              ...
                              <div className="bg-slate-800/50 p-2 rounded">
                                  <label className="text-[9px] text-slate-500 uppercase font-bold block mb-1">Fase (Offset Hora)</label>
                                  <div className="flex items-center gap-2">
                              ...
                              <div className="bg-slate-800/50 p-2 rounded">
                                  <label className="text-[9px] text-slate-500 uppercase font-bold block mb-1">Nível Médio (%)</label>
                                  <div className="flex items-center gap-2">
                  ...
                  {dataSourceConfig.activeSource === TideSourceType.MOCK && (
                      <div className="animate-in fade-in flex flex-col h-full min-h-0">
                           <div className="bg-slate-900/50 p-3 rounded mb-3 border border-slate-700 shrink-0">
                               <label className="text-[9px] text-slate-500 uppercase font-bold block mb-2">Criar Novo Mock</label>
                               <div className="flex gap-2">
                                   <input type="text" value={newMockName} onChange={e=>setNewMockName(e.target.value)} placeholder="Nome do snapshot" className="flex-1 bg-slate-800 border border-slate-600 rounded p-1.5 text-xs text-white"/>
                                   <button onClick={handleSaveMock} className="bg-green-600 hover:bg-green-500 text-white px-3 rounded text-xs font-bold flex items-center gap-2">
                                       <Save size={12}/> Salvar Estado Atual
                                   </button>
                               </div>
                           </div>
                           
                           <label className="text-[9px] text-slate-500 uppercase font-bold block mb-2 shrink-0">Mocks Salvos</label>
                           <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar min-h-0">
                               {savedMocks.map(mock => (
                                   <div key={mock.id} className="bg-slate-800 p-2 rounded border border-slate-700 flex justify-between items-center group hover:border-green-500/50 transition">
                                       <div onClick={() => { setKeyframes(mock.frames); setNotification('success', `Mock "${mock.name}" carregado!`); }} className="cursor-pointer flex-1 min-w-0">
                                           <div className="text-xs font-bold text-slate-200 group-hover:text-green-400 transition truncate">{mock.name}</div>
                                           <div className="text-[9px] text-slate-500 flex items-center gap-2 mt-0.5">
                                               <span className="bg-slate-700 px-1 rounded whitespace-nowrap">{mock.frames.length} pts</span>
                                               <span className="truncate">{mock.description || 'Sem descrição'}</span>
                                            </div>
                                       </div>
                                       <div className="flex gap-1 shrink-0">
                                           <button onClick={() => { setKeyframes(mock.frames); setNotification('success', `Mock "${mock.name}" carregado!`); }} className="text-slate-400 hover:text-green-400 p-1.5 rounded hover:bg-slate-700 transition" title="Carregar">
                                               <FolderOpen size={14}/>
                                           </button>
                                           <button onClick={() => deleteMock(mock.id)} className="text-slate-400 hover:text-red-400 p-1.5 rounded hover:bg-slate-700 transition" title="Excluir">
                                               <Trash2 size={14}/>
                                           </button>
                                       </div>
                                   </div>
          ...
          <div className="xl:col-span-1 bg-slate-800 rounded-lg border border-slate-700 p-4 flex flex-col overflow-hidden min-h-0">
               <h3 className="text-xs font-bold text-white mb-3 flex items-center gap-2 border-b border-slate-700 pb-2 shrink-0">
                   <Sliders size={14} className="text-orange-400"/> Ambiente (Override)
              </h3>
              
              <div className="flex-1 overflow-y-auto space-y-4 px-1 custom-scrollbar min-h-0">
                    <div>
                        <div className="flex justify-between mb-1">
                             <label className="text-[9px] text-slate-500 uppercase font-bold flex items-center gap-1"><Thermometer size={10} className="text-orange-400"/> Temperatura</label>
                    ...
                    <div>
                        <div className="flex justify-between mb-1">
                             <label className="text-[9px] text-slate-500 uppercase font-bold flex items-center gap-1"><Droplets size={10} className="text-blue-400"/> Umidade</label>
                    ...
                    <div>
                        <div className="flex justify-between mb-1">
                             <label className="text-[9px] text-slate-500 uppercase font-bold flex items-center gap-1"><Wind size={10} className="text-slate-300"/> Vento</label>
                    ...
                    <div>
                        <div className="flex justify-between mb-1">
                             <label className="text-[9px] text-slate-500 uppercase font-bold flex items-center gap-1"><Sun size={10} className="text-yellow-400"/> UV Index</label>
                    ...
                    <div>
                        <div className="flex justify-between mb-1">
                             <label className="text-[9px] text-slate-500 uppercase font-bold flex items-center gap-1"><Moon size={10} className="text-purple-400"/> Lua (%)</label>
              ...
              {/* Debug Log Toggler */}
              {apiDebugLog && (
                 <div className="mt-3 pt-2 border-t border-slate-700 shrink-0">
                      <button 
                         onClick={copyLogToClipboard}
                         className="w-full text-[9px] text-slate-500 hover:text-white flex items-center justify-center gap-2 py-1 rounded hover:bg-slate-700 transition"
                      >
                          <Terminal size={10}/> Copiar Log API
                      </button>
                 </div>
              )}
          </div>
